...
export const Layout: React.FC<LayoutProps> = ({ children, onQuickAdd }) => {
  ...
  // Topbar Currency Calculator converter state
  const [calculatorOpen, setCalculatorOpen] = useState(false);
  const [calcInput, setCalcInput] = useState<string>('16');
  const [calcMode, setCalcMode] = useState<'USD_TO_PHP' | 'PHP_TO_USD'>('USD_TO_PHP');

  const currentRate = liveRate || currentUser?.defaultExchangeRate || 58.50;
  const parsedCalcVal = parseFloat(calcInput) || 0;
  const convertedValue = calcMode === 'USD_TO_PHP' 
    ? parsedCalcVal * currentRate 
    : parsedCalcVal / currentRate;
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 glass-header border-b px-4 sm:px-6 flex items-center justify-between shrink-0 select-none relative z-10">
          ...
          <div className="flex items-center gap-2.5 sm:gap-3">
            {/* Currency Converter Quick Calculator */}
            <div className="relative shrink-0">
              <button
                id="topbar-currency-calculator-btn"
                onClick={() => setCalculatorOpen(!calculatorOpen)}
                className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border border-indigo-500/20 dark:border-indigo-400/20 bg-indigo-50/70 dark:bg-indigo-950/40 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 text-[11px] sm:text-xs font-semibold font-mono shadow-xs cursor-pointer transition-all active:scale-95 group select-none shrink-0"
                title="Live USD to PHP Calculator Converter"
              >
                <Calculator className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400 group-hover:rotate-12 transition-transform" />
                <span className="hidden lg:inline text-indigo-600/80 dark:text-indigo-400/80 font-sans">Calc:</span>
                <span className="font-extrabold text-indigo-900 dark:text-indigo-100">
                  {calcMode === 'USD_TO_PHP' ? `$${parsedCalcVal}` : `₱${parsedCalcVal}`} → {calcMode === 'USD_TO_PHP' ? `₱${convertedValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : `$${convertedValue.toFixed(2)}`}
                </span>
              </button>
              ...
              <AnimatePresence>
                {calculatorOpen && (
                  <>
                    <div className="fixed inset-0 z-20" onClick={() => setCalculatorOpen(false)} />
                    <motion.div
                      id="currency-calculator-popover"
                      initial={{ opacity: 0, y: 10, scale: 0.95 }}
                      animate={{ opacity: 1, y: 0, scale: 1 }}
                      exit={{ opacity: 0, y: 10, scale: 0.95 }}
                      className="absolute right-0 mt-2 w-80 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border border-gray-200 dark:border-gray-800 rounded-2xl shadow-2xl p-4 z-30 space-y-3"
                    >
                      {/* Popover Header */}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-gray-400 font-mono">
                              1 USD = ₱{currentRate.toFixed(2)} PHP
                            </p>
                          </div>
                        </div>
                        <button
                      ...
                      {/* Converter Form Input / Output */}
                      <div className="space-y-2.5">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest font-mono">
                            {calcMode === 'USD_TO_PHP' ? 'USD Amount ($)' : 'PHP Amount (₱)'}
                          </span>
                          <button
                            type="button"
                            onClick={() => setCalcMode(calcMode === 'USD_TO_PHP' ? 'PHP_TO_USD' : 'USD_TO_PHP')}
                        ...
                        <div className="relative">
                          <span className="absolute left-3 top-1/2 -translate-y-1/2 font-bold font-mono text-xs text-gray-400">
                            {calcMode === 'USD_TO_PHP' ? '$' : '₱'}
                          </span>
                          <input
                            id="currency-calc-input"
                            type="number"
                            step="any"
                            min="0"
                            value={calcInput}
                            ...
                            className="w-full pl-7 pr-3 py-2 text-sm font-bold font-mono rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
                          />
                        </div>

                        {/* Result output display */}
                        <div className="p-3 rounded-xl bg-indigo-50/70 dark:bg-indigo-950/40 border border-indigo-100 dark:border-indigo-900/50 flex justify-between items-center">
                          <span className="text-xs font-semibold text-indigo-700 dark:text-indigo-300">
                            Equivalent in {calcMode === 'USD_TO_PHP' ? 'PHP' : 'USD'}:
                          </span>
                          <span className="text-sm font-extrabold font-mono text-indigo-900 dark:text-indigo-100">
                            {calcMode === 'USD_TO_PHP' 
                        ...
                        {calcMode === 'USD_TO_PHP' && (
                          <div>
                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest font-mono block mb-1.5">
                              Quick USD Presets
                            </span>
                            <div className="flex flex-wrap gap-1.5">
                              {['5', '10', '16', '20', '50', '100', '500', '1000'].map((preset) => (
                                <button
                                  key={preset}
                                  type="button"
                                  onClick={() => setCalcInput(preset)}
                      ...
                      {/* Footer */}
                      <div className="pt-2 border-t border-gray-100 dark:border-gray-800 flex justify-between items-center text-[9px] font-mono text-gray-400">
                        <span>Wise Exchange Rate Source</span>
                        <span className="text-emerald-500 font-semibold">Live Conversion</span>
                      </div>
                    </motion.div>
                  </>
                )}
